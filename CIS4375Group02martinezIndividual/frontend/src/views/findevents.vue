...
export default {
  ...
  setup() {
    ...
    //added this variable in because it was being used within the options api but there was no variable declared for isLoading
    ...
    const loadData = async () => {
      ...
      } //might have to add something here that makes it where the isLoading.value = false so there isnt a logic error.
    }
    ...
    const handleSubmitForm = async () => {
      ...
      if(searchBy.value === 'Event Name') {
        ...
        if (eventDate.value) {
          try {
            const eventDate = new Date(eventDate.value);
